...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(
        ...
        # l1=torch.jit.annotate(List[int],[]) l1=[]
        # for i in range(len(self.groupers)):
        #     l1.append(i)
        # tp1=tuple(l1)
        #print(len(self.groupers),':',tp1)
        #tp1=()
        #print(new_xyz,' ',self.npoint,' ',len(self.groupers))
        for grouper,mlp in zip(self.groupers,self.mlps):
            ...
            # )  # (B, mlp[-1], npoint, 1)
            new_features=torch.max(new_features,dim=3)[0]
            # new_features = new_features.squeeze(-1)  # (B, mlp[-1], npoint)

            new_features_list.append(new_features)
        #print(new_xyz,' ',torch.cat(new_features_list, dim=1))
