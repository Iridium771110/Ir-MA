...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        idx=torch.ops.pnt2_ops.ball_query(new_xyz, xyz, self.radius, self.nsample)
        #xyz_trans = xyz.transpose(1, 2).contiguous()
        xyz_trans = xyz.permute(0, 2, 1).contiguous()
        #grouped_xyz = grouping_operation(xyz_trans, idx)  # (B, 3, npoint, nsample)
        grouped_xyz = torch.ops.pnt2_ops.grouping(xyz_trans, idx)
        #grouped_xyz -= new_xyz.transpose(1, 2).unsqueeze(-1)
        grouped_xyz -= new_xyz.permute(0, 2, 1).unsqueeze(-1)

        if features is not None:
            #grouped_features = grouping_operation(features, idx)
            grouped_features = torch.ops.pnt2_ops.grouping(features, idx)
            if self.use_xyz:
