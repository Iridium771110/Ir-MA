...
def train(total_epoch,log_path,tr_model,lr_scheduler,tr_dataloader,va_dataloader,optimizer,device):
    ...
    torch.save(tr_model.state_dict(), '/home/dong/WS/test/cls_model_%d.pth' % (total_epoch-1))
...
#   set model & op
    ...
    lr_clip = 1e-5
    #loss_func=F.cross_entropy.to(device)
    ...
    lr_lbmd = lambda index_epoch: max(
                ...
                ** (
                    int(
                        #tr_model.global_step #指参数更新了一次，发生了一次反向传播
                        index_epoch
                        * args.batch_size
