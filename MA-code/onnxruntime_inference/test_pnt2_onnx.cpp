...
#include<iomanip>
/*
This file is for inference test with ONNX Runtime on the CPU
*/
...
bool test_pnt2( Ort::Env &env,
    ...
    // set inference configuration and test
    // Parameters: onnxruntime environment, path to ONNX model, path to points data, path to target labels, 
    //             input name for ONNX model, output name for ONNX model, number of points, batch size, custom operator domain

    // set session options
    Ort::SessionOptions session_options;
    std::cout<<"pnt2 test with default provider"<<std::endl;
    if (custom_op_domain_ptr) session_options.Add(custom_op_domain_ptr); // add the custom operators (domain)
    else std::cout<<"without custom operator"<<std::endl;
    session_options.SetLogSeverityLevel(2);
    // char profile_name[3]; profile_name[0]='b'; profile_name[1]='=';
    // profile_name[2]=b_size+'0';
    // session_options.EnableProfiling(profile_name); // set the profiling file
    ...
    // read the points data, set the input and run test
    ...
    while(labels_input.read((char*)&label[0],sizeof(int64_t))){
        ...
        // generate the input tensor by points data
        input_tensors.clear();
        ...
        // get output data through inference
        ...
        for (int64_t i=0;i<output_dim[0];i++){
            // check the inference result with the target label
            max_f=-1e10;
            ...
            if (output_label==label[i]) correct++; // count the correct result
        }
    }
    points_input.close();
    labels_input.close();
...
int main(int argc, char** argv){
    ...
    Ort::Env env_=Ort::Env(ORT_LOGGING_LEVEL_INFO,"Default"); // set the onnxruntime environment
    
    // register the custom operators with ONNX Runtime 
    // please ensure the consistency of domain name and operator name
    ...
    std::cout<<test_pnt2(env_,model_path,data_path,label_path,"points","results",4096,batch_size,custom_op_domain)<<std::endl; // do inference
