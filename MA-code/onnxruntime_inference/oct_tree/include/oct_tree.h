...
#include <vector>
/*
This file is the head file for octree
It includes the fundemental functions for building and searching
Some computation based on sign bit aims avoiding the conditional branch
*/
...
struct Point{
    ...
    float y;
    float z;
};// point data includes 3-dimensional values
...
struct Oct_Node{
    ...
    Oct_Node* child[8];
};// Octree node type includes space range and child nodes ptr

class Oct_Tree{
    // Octree class
    public:
    Oct_Node* Root=new Oct_Node; // root node of octree
    float resolution; // resolution of octree
    float x_max,x_min,y_max,y_min,z_max,z_min; // range of the entire space, but seems the same as the range of root node and not used
    int valid_node_num; // number of non-empty leaf nodes

    Oct_Tree(float reso,float x0, float x1,float y0,float y1,float z0,float z1){
        // initialize the root node and the octree
        //Parameters: resolution of octree and the data range (range of the entire space)
    ...
    void Create_Node(Oct_Node* father, Point point){
        // create the tree node for the point which does not matches the current valid leaf nodes
        //Parameters: The current tree node, point data

        float dist_x=father->max_range.x-father->min_range.x;
        float dist_y=father->max_range.y-father->min_range.y;
        float dist_z=father->max_range.z-father->min_range.z; // calculate the side length of current node
        float diff_x=this->resolution-dist_x;
        float diff_y=this->resolution-dist_y;
        float diff_z=this->resolution-dist_z; // check if the side length larger than the resolution
        char divide_x= ((((char*)&diff_x)[sizeof(float)-1]>>7) & (0x01));
        char divide_y= ((((char*)&diff_y)[sizeof(float)-1]>>7) & (0x01));
        char divide_z= ((((char*)&diff_z)[sizeof(float)-1]>>7) & (0x01)); // get the sign bit
        // minus = 1, reso < dist, the space is divisible in this side

        if (divide_x|divide_y|divide_z){ 
            // the space is at least divisible in one side
            ...
            diff_z=center_z-point.z; // calculate the difference between block center and the point

            char child_x=((((char*)&diff_x)[sizeof(float)-1]>>7) & (0x01));
            char child_y=((((char*)&diff_y)[sizeof(float)-1]>>7) & (0x01));
            char child_z=((((char*)&diff_z)[sizeof(float)-1]>>7) & (0x01)); // get the sign bit
            // minus = 1, center < point
            char child_index;
            child_index=((child_x&divide_x)<<2)|((child_y&divide_y)<<1)|(child_z&divide_z); //calculate the child index using sign bits
            // if this side is divisible, then the child index in this side depends on the difference between block center and the point
            // else the child index in this side is 0

            Oct_Node* new_child=new Oct_Node; // set the new child node for current node
            ...
            new_child->max_range.z=father->max_range.z-(divide_z&(1-child_z))*dist_z/2.0f; // set the space range of the new child node using sign bit
            // the range in indivisible side inherits the range from the current node (father node)
            // the range in divisible side will be calculated

            new_child->point_num=0;
            new_child->is_leaf=false;
            for (int i=0;i<8;i++) new_child->child[i]=nullptr; // set the parameters of new child node
            father->child[child_index]=new_child; // mount the new child node on the current node (father node)
            new_child=nullptr;
            Create_Node(father->child[child_index],point); // repeat the process until the node is indivisible (achieve the resolution)
            //std::cout<<this->Root->is_leaf<<"dd"<<std::endl;
            father=nullptr;
        }
        else{
            // can not divide any more
            father->is_leaf=true;
            father->label=this->valid_node_num;
            father->point_num++; // set the node parameters as a vaild leaf node
    ...
    int Creat_Tree(Point point){
        // create the octree depends on the point data
        // given is the point data for octree, return is the label of the corresponding valid leaf node
        ...
        while (!cur_node->is_leaf){
            // search the corresponding node until achieve a valid leaf node
            ...
            // calculate the child node index using sign bit to avoid conditional branch

            if(cur_node->child[child_index]==nullptr) {
                // the corresponding node is not built, then build the node till a valid leaf node
                ...
                cur_node=nullptr;
                //std::cout<<this->Root->is_leaf<<std::endl;
                return label; // new vaild leaf node, directly return the label
            }
        ...
        cur_node=nullptr;
        return label; // existed valid leaf node, return its label
    };

    int Get_position_label(Point point){
        // search the point in the octree and return the label of corresponding valid leaf node
        // given is the point data, return is the label
        ...
        while (!cur_node->is_leaf){
            ...
            if(cur_node->child[child_index]==nullptr) {
                cur_node=nullptr;
                return -1; // no valid leaf node founded, this point can not be covered by this octree, return an invalid label
            }
            cur_node=cur_node->child[child_index];
        }
        int label=cur_node->label;
        cur_node=nullptr;
        return label; // a valid leaf node founded, return the label of this node
    };

    void Get_all_valid(OCT_TREE::Oct_Node** nodes, int valid_leaf_num){
        // get all valid leaf nodes and check the number of nodes
        // Parameters: Array to store the node ptr, expected number of valid leaf nodes
        ...
        while(!to_search.empty()){
            // find all valid leaf nodes and store them in the array
    ...
    void Delete_Node(Oct_Node* root){
        // delete the subtree for the given root node
        if (root->is_leaf){
            return;
        }
        else{
